...
Pregunta1 <-dbGetQuery(portaldb,"SELECT count(reviewid), artist FROM reviews GROUP by (artist) ORDER by (count(reviewid)) desc limit 2;")

Pregunta2 <-dbGetQuery(portaldb,"SELECT avg(score) FROM reviews GROUP by pub_year BETWEEN 2010 and 2014  HAVING pub_year BETWEEN 2010 and 2014 ;")
